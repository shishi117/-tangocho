// FR010 間隔反復（Leitner 5箱）。純粋関数。SM-2等へ差し替えるならこのファイルを置換する。

// 出題重み比 box1:box2:box3:box4:box5 = 16:8:4:2:1（苦手ほど高頻度）。
export const BOX_WEIGHTS = [16, 8, 4, 2, 1];
...
export function updateBox(box, correct) {
  return correct ? clampBox(clampBox(box) + 1) : 1;
}

// 箱の重みで1枚を抽選。全カードが同一箱なら重みが等しくなり自動的に一様抽選になる（FR010の例外を包含）。
// rng は [0,1) を返す関数。既定 Math.random。テスト時はシード付きRNGを注入する。
/**
 * @template {{ box: number }} T
 * @param {readonly T[]} cards
 * @param {() => number} [rng]
 * @returns {T}
 */
export function pickCard(cards, rng = Math.random) {
  if (cards.length === 0) {
